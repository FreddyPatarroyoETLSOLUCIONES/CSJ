...
export class FiltrosAvanzadosComponent implements OnInit {
  ...
  resultConsult:any;
  chckSeccion: String;
  chckJurisprudencia:any;
  chckJurisprudenciaValue:boolean=false;
  ...
  historicoPaginaActual: number = 1;
  ...
  this.stateArray[0] = "Inactivo";
  this.stateArray[1] = "Activo";
...
public getParamToolTips(cantidadPorPagina:number , historicoPaginaActual:number) {
  ...
  this.elasticService.getParamLogos(auxToken , this.historicoRegistrosPagina , this.historicoPaginaActual , nameSearchTmp , tipoFiltro).subscribe(
      (respuesta: any) => {
          ...
          (Object.keys(respuesta.results) as (keyof typeof respuesta.results)[]).forEach((key, index) => {
            this.listStateArray[respuesta.results[key].internal_id] = false;
...
updateDataByIdInternalId(){
  ...
  updateStatusByIdInternalId(internal_id:any , status:any){

    var auxToken = sessionStorage.getItem('token');
    var statusParam = status ? false : true;

    let jsonString = '{ "internal_id": "' + internal_id +'" ,  "status": "' + statusParam +'" }';
  
    let json = JSON.parse(jsonString);
   
    this.elasticService.putParamLogosStatus(auxToken, internal_id, json).subscribe(
      (respuesta: any) => {
        this.flagModalEditar = false;
        this.changePaginadoFiltroGeneralBusqueda(this.historicoPaginaActual);
        this.toggleShowStatusEvent(internal_id);

      }); 
    }

  toggleShowStatusEvent(idElement:any): void {
    this.listStateArray[idElement] = !this.listStateArray[idElement];
  }

  toggleShowChckJurisprudenciaValue(): void {
    this.chckJurisprudenciaValue = !this.chckJurisprudenciaValue;
  }
}
